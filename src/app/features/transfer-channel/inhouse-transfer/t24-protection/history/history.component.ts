...
import {UPDATE_AT_FROM, UPDATE_AT_TO, UPDATE_BY} from '../model/constant';

import {T24ProtectionService} from '../../services/t24-protection.service';
import {IHistory, IPageable} from '../model/interface';
...
export class HistoryComponent extends ComponentAbstract {
  displayedColumns: string[] = [
    'stt', 'typeUpdate', 'createdBy', 'createdAt', 'updatedBy', 'updatedAt', 'reason', 'actions',
  ];
  ...
  handleGetList(): Observable<HttpResponse<IPageable<IHistory>>> {
    ...
    return this.t24ProtectionService.getHistoryConfigList(this.options.params).pipe(
  ...
  viewDetailHistory(element: IHistory) {
    this.dialogService.componentDialog(DetailHistoryComponent, {
      name: 'DetailHistoryComponent',
      width: '60%',
      maxHeight: '97vh',
      maxWidth: '97vw',
      data: {
