...
import { URL, DISPUTE_AMOUNT, DISPUTE_CLAIM_CODE, DISPUTE_MESSAGE, DISPUTE_SUBJECT, DISPUTE_TYPE_CODE, SELECT_DISPUTE_CLAIM_CODE, SELECT_DISPUTE_TYPE_CODE, REFUND_AMOUNT, REFUND_CREATED_DATE, REFUND_CODE, REFUND_ACCOUNT, REFUND_DESCRIPTION, SELECT_DISPUTE_STATUS, DISPUTE_STATUS } from '../modal/constant';
import { approveDisputeStatusEnum, typeTransactionDisputeEnum } from '../modal/enum';
...
@Component({
  selector: 'app-reply-edit-dispute',
  templateUrl: './reply-edit-dispute.component.html',
  styleUrls: ['./reply-edit-dispute.component.scss']
})
export class ReplyEditDisputeComponent extends ComponentAbstract {
      ...
      forkJoin({
        ...
        switchMap(({ transactionOrigin, dispute }) => {
          ...
          if (dispute.status === 200 && dispute.data.disputes && dispute.data.disputes.length > 0) {
            ...
            let disputeStatus = this.dataTransactionDisputeResponse?.disputeStatus ?? null;
            ...
            if (disputeStatus && disputeStatus in mapStatus) {
              disputeStatus = mapStatus[disputeStatus];
            }

            this.form.patchValue({
              disputeSubject: this.dataTransactionDisputeResponse?.disputeSubject ?? null,
              disputeMessage: this.dataTransactionDisputeResponse?.disputeMessage ?? null,
              disputeStatus:disputeStatus,
              // disputeAmount: this.dataTransactionDisputeResponse.disputeAmount ?? null,
            })
            // if(this.dataTransactionDisputeRequest?.disputeAmount) {
            //   this.showDisputeAmount = true 
            //   this.$disputeAmount.required = true
            //   this.form.patchValue({
            //     disputeAmount: this.dataTransactionDisputeResponse?.disputeAmount
            //   })
            //   this.cdr.detectChanges();
            //   setTimeout(() => {
            //     if (this.amountDispute && this.amountDispute.inputElement) {
            //       this.amountDispute.inputElement.nativeElement.value = this.convertMoney(this.dataTransactionDisputeResponse?.disputeAmount);
            //     } else {
            //       console.warn('amountDispute vẫn chưa được gán');
            //     }
            //   });
            // }
            this.fileName = this.dataTransactionDisputeResponse?.fileName ? this.dataTransactionDisputeResponse?.fileName : null
          }
  ...
  modalUpdateDispute() {
    this.dialogService.dformconfirm({
      label: 'Chỉnh sửa yêu cầu phản hồi tra soát',
      acceptBtn: 'Đồng ý',
      closeBtn: 'Đóng',
      innerHTML: 'Xác nhận chỉnh sửa yêu cầu phản hồi tra soát?'
    }, (result: any) => {
  ...
  async updateTransactionDispute() {
    ...
    this.indicator.showActivityIndicator();
    this.searchDisputeService.updateReplyTransactionDispute(this.createDisputeResponse).pipe(
      takeUntil(this.ngUnsubscribe),
  ...
  loadBtn() {
    let listBtn = []
    if (this.dataTransactionOrigin && this.dataTransactionDisputeResponse && this.dataTransactionDisputeResponse.approveDisputeStatus !== approveDisputeStatusEnum.APPROVED) {
      listBtn.push(BUTTON_SAVE)
    }
    if (this.dataTransactionOrigin && this.dataTransactionDisputeResponse && this.dataTransactionDisputeResponse.approveDisputeStatus === approveDisputeStatusEnum.WAITING) {
