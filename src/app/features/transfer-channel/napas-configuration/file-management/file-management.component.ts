...
import { ComponentAbstract, MessageSeverity, DformDialogService } from '@shared-sm';
...
export class FileManagementComponent extends ComponentAbstract {
  ...
  QueryData() {
    ...
    this.fileManagementService.queryFile(this.options.params, this.requestParams).pipe(
      ...
      if (res && res.status === 200) {
        ...
        const data = res.data.fileOutputs.map((obj, index) => {
  ...
  previewFile(element) {
    this.indicator.showActivityIndicator();
    this.fileManagementService.previewFile(element.id).pipe(
      finalize(() => {
        this.indicator.hideActivityIndicator();
      }),
      takeUntil(this.ngUnsubscribe)
