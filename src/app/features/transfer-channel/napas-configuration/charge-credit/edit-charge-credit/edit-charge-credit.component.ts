...
import { approveDisputeStatusEnum } from '../../search-dispute/modal/enum';
...
import { ICreateChargeCredit, IParamsSearch, IResponseTransactionDispute, IResponseTransactionOriginal, IParamsSearchTransactionOrigin, IResponseChargeCredit, IUpdateChargeCredit } from '../modal/interface';

@Component({
  selector: 'app-edit-charge-credit',
  templateUrl: './edit-charge-credit.component.html',
  styleUrls: ['./edit-charge-credit.component.scss']
})
export class EditChargeCreditComponent extends ComponentAbstract {
  ...
  requestChargeCredit: IUpdateChargeCredit | null = null;
    ...
    forkJoin([dispute, chargeCreditApi]).pipe(
      ...
      switchMap(([dispute, chargeCreditData]) => {
        ...
        if(chargeCreditData && chargeCreditData.data.chargeCredits.length > 0) {
          ...
          this.form.patchValue({amount: this.dataChargeCredit?.amountValue})
          this.amountChargeCredit.inputElement.nativeElement.value = this.convertMoney(this.dataChargeCredit?.amountValue.toString());
        }
  ...
  modalAddAccount() {
    this.dialogService.dformconfirm({
      label: 'Chỉnh sửa yêu cầu báo có',
      acceptBtn: 'Đồng ý',
      closeBtn: 'Đóng',
      innerHTML: 'Xác nhận chỉnh sửa yêu cầu báo có?'
    }, (result: any) => {
  ...
  async createTransactionOrigin() {
    ...
    this.indicator.showActivityIndicator();
    this.chargeCreditService.updateChargeCredit(this.requestChargeCredit, this.dataChargeCredit.id).pipe(
      takeUntil(this.ngUnsubscribe),
      finalize(() => this.indicator.hideActivityIndicator())
    ).subscribe((res) => {
      if (res && res.status === 200) {
        this.toastr.showToastr(
          'Chỉnh sửa yêu cầu báo có thành công',
          'Thông báo!',
  ...
  async createRequestDispute(): Promise<IUpdateChargeCredit> {
    return {
      disputeId: this.idDispute,
      amount: Number(this.form.get('amount')?.value) ? Number(this.form.get('amount')?.value) : null,
      currency: '704', //VND
      status: approveDisputeStatusEnum.WAITING,
    }
  }
