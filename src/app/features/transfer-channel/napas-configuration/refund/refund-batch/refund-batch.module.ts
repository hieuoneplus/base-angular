import { NgModule } from '@angular/core';
import { CommonModule } from '@angular/common';
import { TranslateModule } from '@ngx-translate/core';
import { SharedSMModule } from '@shared-sm';
import { MatIconModule } from '@angular/material/icon';
import { MatCheckboxModule } from '@angular/material/checkbox';
import { MatTooltipModule } from '@angular/material/tooltip';
import { MatTableModule } from '@angular/material/table';
import { ThemeModule } from 'src/app/theme/theme.module';
import { MaterialModule } from '../../../../../shared/material.module';
import { RefundBatchComponent } from './refund-batch.component';
import { RefundBatchRoutingModule } from './refund-batch-routing.module';
import { DetailRefundBatchComponent } from './detail-refund-batch/detail-refund-batch.component';


@NgModule({
  declarations: [
    RefundBatchComponent,
    DetailRefundBatchComponent
  ],
  imports: [
    CommonModule,
    RefundBatchRoutingModule,
    ThemeModule,
    TranslateModule,
    SharedSMModule,
    MatIconModule,
    MatTableModule,
    MatCheckboxModule,
    MatTooltipModule,
    MaterialModule,
  ],
})
export class RefundBatchModule { }
