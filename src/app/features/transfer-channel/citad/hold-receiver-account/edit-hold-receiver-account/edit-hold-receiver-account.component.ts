...
import { IRequestUpdateHoldReceiverAccount } from '../modal/interface';

@Component({
  selector: 'app-edit-hold-receiver-account',
  templateUrl: './edit-hold-receiver-account.component.html',
  styleUrls: ['./edit-hold-receiver-account.component.scss']
})
export class EditHoldReceiverAccountComponent extends ComponentAbstract {

  requestUpdateHoldReceiverAccount: IRequestUpdateHoldReceiverAccount = {
    key: KeyConfigCitad.hold_receiver_account,
    value: {
      $type: "hold_receiver_account",
      holdReceiverAccounts: []
    },
    reason: ""
  }

  initialHoldReceiverAccounts: string[] = [];
  ...
  isValidFormControl(controlName: string, error?: string): string | boolean {
    ...
    if (control.value.text) {
      if (forbiddenChars.test(control.value.text) || char10orChar13.test(control.value.text) || vietnamese.test(control.value.text)) {
    ...
    this.holdReceiverAccountService.getHoldReceiverAccount().pipe(
      ...
      next: (res) => {
        if (res && res.status === 200) {
          const holdReceiverNamePatterns = res.data?.value?.holdReceiverAccounts || [];
          this.initialHoldReceiverAccounts = [...holdReceiverNamePatterns];
          res.data?.value?.holdReceiverAccounts.reverse().forEach(item => {
  ...
  modalUpdateHoldReceiverAccount() {
    ...
    if (this.mFormGroup.invalid || findDuplicates.length > 0 || values.some(value => forbiddenChars.test(value) || char10orChar13.test(value) || vietnamese.test(value))) {
      this.mFormGroup.markAllAsTouched();
      return;
    }

    this.dialogService.dformconfirm({
      label:'Chỉnh sửa cấu hình tài khoản không đủ điều kiện auto treo',
      title: 'Lý do',
  ...
  callUpdateConfig(reason: string) {
    ...
    this.requestUpdateHoldReceiverAccount.reason = reason.trim();
    this.requestUpdateHoldReceiverAccount.value.holdReceiverAccounts = valueMbsignal
    this.holdReceiverAccountService.updateHoldReceiverAccount(this.requestUpdateHoldReceiverAccount).pipe(
      finalize(() => {
        this.indicator.hideActivityIndicator()
      }),
      takeUntil(this.ngUnsubscribe)
  ...
  compareData() {
    ...
    const areEqual = JSON.stringify(currentValues) === JSON.stringify(this.initialHoldReceiverAccounts).toUpperCase().trim();
  ...
  onClickBtn($event?: any): void {
    switch ($event) {
      ...
      case TYPE_BTN_FOOTER.TYPE_SAVE:
        this.modalUpdateHoldReceiverAccount()
        break;
    }
  }
