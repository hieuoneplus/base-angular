...
import { CURRENCY, DEBIT_VALUE_DATE, MAX_AMOUNT, MIN_AMOUNT, T24_REFERENCE_NUMBER, STATUS_OUT, SHOWING_CONDITION, STATUS_LABEL_TRANSACTION_OUT } from '../constant';
...
@Component({
  selector: 'app-transaction-manager-out',
  templateUrl: './transaction-manager-out.component.html',
  styleUrls: ['./transaction-manager-out.component.scss']
})
export class TransactionManagerOutComponent extends ComponentAbstract {

  displayedColumns: string[] = [
    'check', 'stt', 'transactionId', 't24ReferenceNumber', 'gateway', 'debitValueDate', 'debitAccountNo', 'creditAccountNo', 'amountCredited', 'debitCurrency', 'partnerStatus', 'wrappedStatus', 'message', 'actions'
  ];

  $t24ReferenceNumber = T24_REFERENCE_NUMBER();
  $isRetryable = SHOWING_CONDITION();
  ...
  protected componentInit(): void {
    this.form = this.itemControl.toFormGroup([
      this.$t24ReferenceNumber, this.$isRetryable, this.$debitValueDate, this.$minAmount, this.$maxAmount, this.$status, this.$currency
    ]);
    this.enableActions(ModuleKeys.citad_transactions_outward)
    this.search();
    ...
    this.transactionManagerService.exportTransactionCitad(this.options.params, type).pipe(
      ...
      next: (res) => {
        if (res instanceof HttpResponse) {
          ...
          a.href = url;
          a.download = 'TruyVanDienDi_' + moment(this.form.get('debitValueDate')?.value).format('YYYY-MM-DD') + '.xlsx'
          a.click();
  ...
  viewDetail(element: any) {
    this.goTo('pmp_admin/transfer-channel/citad/transaction-manager-out/detail', { "transactionId": element.transactionId })
  }

  getLabel($status: any) {
    const status = (STATUS_LABEL_TRANSACTION_OUT || []).find(item => item.key === $status);
    if (status) {
