...
@Component({
  selector: 'citad-search-blacklist-page',
...
export class SearchBlacklistComponent extends ComponentAbstract {
  ...
  constructor(
    ...
    private whitelistService: CitadBlacklistService
  ) {
    super(injector);
  }
  ...
  protected componentInit(): void {
    ...
    this.enableActions(ModuleKeys.citad_blacklist_accounts)
    this.search();
  }
  ...
  onClickCreateAccount() {
    this.goTo('/pmp_admin/transfer-channel/citad/blacklist-account/add')
  }

  onClickEdit(element: IAccountContent) {
    console.log("EDIT ", element);
    this.goTo('/pmp_admin/transfer-channel/citad/blacklist-account/edit', { id: element.id });
  }
  ...
  exportExcel() {
    this.indicator.showActivityIndicator()
    this.whitelistService.exportBlacklistAccounts().pipe(
      finalize(() => {
      ...
      next: (res) => {
        if (res instanceof HttpResponse) {
          ...
          a.href = url;
          a.download = 'Tai_khoan_khong_duoc_ghi_co_tu_dong.xlsx'
          a.click();
  ...
  onClickHistory() {
    this.goTo('/pmp_admin/transfer-channel/citad/blacklist-account/histories')
  }
}
