...
import { UPDATE_AT_FROM, UPDATE_AT_TO, UPDATE_BY } from '../../bankcode-cardbin/modal/constant';
...
import { DetailHistoryTransferChannelLimitComponent } from './detail-history/detail-history.component';
...
export class HistoryComponent extends ComponentAbstract {
  displayedColumns: string[] = [
    'stt', 'transferChannel', 'currency', 'minAmount', 'maxAmount', 'fragmentMaxAmount', 'fragmentAmount',
    'active', 'typeUpdate', 'updatedAt','updatedBy', 'reason', 'actions',
  ];
  ...
  handleGetList(): Observable<HttpResponse<IPageable<IHistory>>> {
    ...
    return this.transferChannelLimitService.getHistoryConfigList(this.queryParams?.transferChannelId, this.options.params).pipe(
  ...
  viewDetailHistory(element: IHistory) {
    this.dialogService.componentDialog(DetailHistoryTransferChannelLimitComponent, {
      name: 'DetailHistoryTransferChannelLimitComponent',
      width: '60%',
      maxHeight: '97vh',
      maxWidth: '97vw',
      data: {
